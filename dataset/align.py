...
def cropRange(mat, img_shape, center):
    ...
    for point in transformed_corner_point:
        x, y = point
        w = max(w, abs(tranformed_center[0] - x),
                abs(tranformed_center[1] - y))
...
def inverseImage(aligned_img, aligned_size, original_shape, r_mat):
    resize_img = cv2.resize(aligned_img, aligned_size,
                            interpolation=cv2.INTER_NEAREST)
    r_mat = np.vstack([r_mat, np.array([0, 0, 1])])
    r_mat_inv = np.linalg.inv(r_mat)
    transformed_img = cv2.warpAffine(
        resize_img, r_mat_inv[:2], (original_shape[1], original_shape[0]))
...
def alignImage(img, toRGB=True):
    ...
    aligned_img = cv2.resize(alignment, (512, 512),
...
def inverseTensor(aligned_imgs, aligned_predicteds, aligned_masks, aligned_size, original_shape, r_mat, unseen=False):
    ...
    for b in range(batch_size):
        aligned_img, aligned_predicted, aligned_mask = aligned_imgs[
            b], aligned_predicteds[b], aligned_masks[b]
        ...
        aligned_img = cv2.resize(
            aligned_img, aligned_size, interpolation=cv2.INTER_NEAREST)
        aligned_img = cv2.warpAffine(aligned_img, r_mat_inv, original_shape)
        aligned_img = np.transpose(aligned_img, (2, 0, 1))

        aligned_predicted = cv2.resize(
            aligned_predicted, aligned_size, interpolation=cv2.INTER_NEAREST)
        aligned_predicted = cv2.warpAffine(
            aligned_predicted, r_mat_inv, original_shape)

        if not unseen:
            for i in range(19):
                new_aligned_mask = cv2.resize(
                    aligned_mask[i], aligned_size, interpolation=cv2.INTER_NEAREST)
                aligned_mask[i] = cv2.warpAffine(
                    new_aligned_mask, r_mat_inv, original_shape)
    ...
    # torch.from_numpy(new_aligned_imgs).cuda()
    aligned_imgs = torch.stack(new_aligned_imgs, dim=0).cuda()
    # torch.from_numpy(new_aligned_predicteds).cuda()
    aligned_predicteds = torch.stack(new_aligned_predicteds, dim=0).cuda()
...
if __name__ == '__main__':
    ...
    # Aligned image
    # Change to alignImage(img, False) if the input image is RGB
    aligned_img, aligned_size, r_mat = alignImage(img)
    cv2.imwrite(OUTPUT_IMAGE, aligned_img)

    # Transform back
    transformed_img = inverseImage(
        aligned_img, aligned_size, (1024, 1024), r_mat)
